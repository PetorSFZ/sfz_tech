...
Message: %s
	...
	SFZ_WARNING("sfzGL",
		MESSAGE_FORMAT,
		debugSourceEnumToString(source),
		debugTypeEnumToString(type),
		id,
		debugSeverityEnumToString(severity),
		message);
...
void setupDebugMessages(Severity acceptLevel, Severity breakLevel) noexcept
{
	if (!glewIsExtensionSupported("GL_ARB_debug_output")) {
		SFZ_ERROR("sfzGL", "GL_ARB_debug_output is not supported on this configuration!");
		return;
	}
