...
{
	// We don't care about 4th components
	origin.w = 0.0f;
	dir.w = 0.0f;
	up.w = 0.0f;

	Vector zAxis = -normalize(dir); // Away from screen
	Vector xAxis = normalize(cross(up, zAxis)); // To the right
	Vector yAxis = cross(zAxis, xAxis); // Up
	...
	tmp.m[1] = xAxis.y;
	tmp.m[2] = xAxis.z;
	tmp.m[3] = -dot(xAxis, origin);

	tmp.m[4] = yAxis.x;
	tmp.m[5] = yAxis.y;
	tmp.m[6] = yAxis.z;
	tmp.m[7] = -dot(yAxis, origin);

	tmp.m[8] = zAxis.x;
	tmp.m[9] = zAxis.y;
	tmp.m[10] = zAxis.z;
	tmp.m[11] = -dot(zAxis, origin);
...
static Matrix createProjectionMatrixInternal(
	float l, float b, float r, float t, float n, float f) noexcept
{
	Matrix tmp;
...
{
	float yFov = DEG_TO_RAD * yFovDeg;
	float yMax = std::tan(yFov * 0.5f) * zNear;
	float xMax = yMax * aspectRatio;
	return createProjectionMatrixInternal(-xMax, -yMax, xMax, yMax, zNear, zFar);
}
