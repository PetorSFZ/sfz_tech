...
void Mouse::update(int windowWidth, int windowHeight, const DynArray<SDL_Event>& events) noexcept
{
	// Pre-processing
	...
	const float scale = 1.0f / (float)windowWidth;
	...
	position = vec2{(float)mouseX, (float)(windowHeight - mouseY)} * scale;
}
