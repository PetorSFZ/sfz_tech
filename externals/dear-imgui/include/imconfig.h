...
// Don't allow default allocators, must replace with your own.
...
#define IM_VEC2_CLASS_EXTRA                                                 \
        ImVec2(const sfz::vec2& f) { x = f.x; y = f.y; }                       \
        operator sfz::vec2() const { return sfz::vec2(x,y); }

#define IM_VEC4_CLASS_EXTRA                                                 \
        ImVec4(const sfz::vec4& f) { x = f.x; y = f.y; z = f.z; w = f.w; }     \
        operator sfz::vec4() const { return sfz::vec4(x,y,z,w); }
